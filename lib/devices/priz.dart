...
class PrizDevice extends BaseDevice {
  ...
  }) : super(type: DeviceType.socket.typeCode);

  PrizDevice.fromMap(Map<String, dynamic> map) : super.fromMap(map) {
  ...
  void handleIncomingValue(dynamic value) {
    ...
    if (value is Map && value.containsKey('com')) {
      ...
      final int? val = int.tryParse(value['val']?.toString() ?? '');
      if (val != null) {
    ...
    if (value is Map && value.containsKey('low') && value.containsKey('high')) {
      int newLow = int.tryParse(value['low'].toString()) ?? 0;
      int newHigh = int.tryParse(value['high'].toString()) ?? 0;
      groupLoadingStates.clear();
      groupLow.value = newLow;
      groupHigh.value = newHigh;
      if (!_isGroupsDialogOpen && value['is_passive_update'] != true) {
        _showGroupsDialog();
      }
      return;
    }
    ...
    if (value is Map && (value.containsKey('scene_status') || value['com'] == 'set_scene')) {
      ...
      if (value.containsKey('scene_status')) {
        _showSceneValueEditDialog(sId, val);
      }
      return;
    }
  ...
  void _sendGetScenarioCommand(int scenarioId) {
    _sendToOutbox({
      "com": "get_scene",
      "adres": id,
      "kanal": channel,
  ...
  void _sendScenarioValueDelete(int scenarioId) {
    _sendToOutbox({
      "com": "del_scene",
      "adres": id,
      "kanal": channel,
  ...
  void _sendScenarioValueUpdate(int scenarioId, int val) {
    _sendToOutbox({
      "com": "set_scene",
      "adres": id,
      "kanal": channel,
  ...
  void _sendDaliConfigCommand(String command, int value) {
    _sendToOutbox({
      "com": command,
      "adres": id,
      "kanal": channel,
  ...
  void _sendDaliGetConfigCommand(String command) {
    _sendToOutbox({
      "com": command,
      "adres": id,
      "kanal": channel,
  ...
  void _showStatusDialog() {
    ...
    final List<String> statusLabels = [
      'status_driver_error'.tr, 'status_lamp_error'.tr, 'status_lamp_on'.tr, 'status_limit_error'.tr,
      'status_fade_running'.tr, 'status_reset_values'.tr, 'status_wrong_address'.tr, 'status_power_on'.tr,
    ];

    Get.dialog(
      AlertDialog(
        title: Text("${name ?? 'socket'.tr} - ${'status'.tr}"),
        content: Obx(() => Column(
          ...
          children: List.generate(statusLabels.length, (index) {
            ...
            return CheckboxListTile(
              ...
              value: isSet, onChanged: null, dense: true, controlAffinity: ListTileControlAffinity.leading,
            );
          }),
        )),
        actions: [
          TextButton(onPressed: () => _sendQStatusCommand(), child: Text("refresh".tr.toUpperCase())),
  ...
  void _showGroupsDialog() {
    ...
    Get.dialog(
      AlertDialog(
        title: Text("${name ?? 'socket'.tr} - ${'groups'.tr}"),
        ...
        content: SizedBox(
          ...
          child: Obx(() {
            ...
            return GridView.builder(
              ...
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 3.2, crossAxisSpacing: 5, mainAxisSpacing: 5),
              itemCount: userManager.activeGroups.length,
              itemBuilder: (context, index) {
                final group = userManager.activeGroups[index];
                return Obx(() {
                  bool isSelected = group.id < 8 ? (groupLow.value & (1 << group.id)) != 0 : (groupHigh.value & (1 << (group.id - 8))) != 0;
                  bool isLoading = groupLoadingStates[group.id] ?? false;
                  return InkWell(
                    onTap: isLoading ? null : () {
                      bool newVal = !isSelected;
                      if (group.id < 8) { if (newVal) groupLow.value |= (1 << group.id); else groupLow.value &= ~(1 << group.id); }
                      else { int bitPos = group.id - 8; if (newVal) groupHigh.value |= (1 << bitPos); else groupHigh.value &= ~(1 << bitPos); }
                      _sendSetGroupsCommand(group.id, newVal ? 1 : 0);
                    },
                    child: Container(
                      padding: const EdgeInsets.symmetric(horizontal: 4),
                      decoration: BoxDecoration(borderRadius: BorderRadius.circular(8), color: isLoading ? theme.colorScheme.primary.withValues(alpha: 0.05) : null),
                      child: Row(
                        children: [
                          SizedBox(width: 32, height: 32, child: isLoading ? const Padding(padding: EdgeInsets.all(8.0), child: CircularProgressIndicator(strokeWidth: 2)) : Checkbox(value: isSelected, onChanged: (bool? val) {
                            if (val == null) return;
        ...
        actions: [
          TextButton(onPressed: () => _sendGetGroupsCommand(), child: Text("refresh".tr.toUpperCase())),
          TextButton(onPressed: () => Get.back(), child: Text("close".tr.toUpperCase(), style: TextStyle(color: theme.colorScheme.primary))),
        ],
      ),
    ).then((_) { _isGroupsDialogOpen = false; groupLoadingStates.clear(); });
  }
  ...
  void _showScenariosDialog() {
    ...
    Get.dialog(
      AlertDialog(
        ...
        title: Text("${name ?? 'socket'.tr} - ${'scenarios'.tr}", style: TextStyle(color: theme.colorScheme.onSurface, fontSize: 18, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
        contentPadding: const EdgeInsets.fromLTRB(15, 20, 15, 10),
        content: SizedBox(
          width: 350,
          child: Obx(() {
            if (userManager.activeScenes.isEmpty) return Center(child: Padding(padding: const EdgeInsets.all(20.0), child: Text('no_users_found'.tr, style: TextStyle(color: theme.colorScheme.onSurface.withValues(alpha: 0.6)))));
            return GridView.builder(
              shrinkWrap: true,
              ...
              itemBuilder: (context, index) {
                ...
                return ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: theme.colorScheme.surface, foregroundColor: theme.colorScheme.onSurface, elevation: 1, padding: const EdgeInsets.symmetric(horizontal: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: BorderSide(color: Colors.grey.withValues(alpha: 0.5)))),
                  onPressed: () { _sendGetScenarioCommand(scene.id); _showSceneValueEditDialog(scene.id, scenarioValues[scene.id] ?? 0); },
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(scene.name, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis),
                      Obx(() {
                        final val = scenarioValues[scene.id];
                        if (val == null) return const SizedBox.shrink();
                        return Text("Deger: $val", style: TextStyle(fontSize: 9, color: theme.colorScheme.primary.withValues(alpha: 0.7), fontWeight: FontWeight.normal));
                      }),
                    ],
                  ),
                );
  ...
  void _showSceneValueEditDialog(int sceneId, int initialValue) {
    ...
    Get.dialog(
      AlertDialog(
        backgroundColor: theme.scaffoldBackgroundColor,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: Text("${'scenarios'.tr} #$sceneId", style: TextStyle(color: theme.colorScheme.onSurface, fontSize: 18, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Obx(() => Text("%${((tempVal.value / 254) * 100).toInt()}", style: TextStyle(color: theme.colorScheme.primary, fontSize: 24, fontWeight: FontWeight.bold))),
            const SizedBox(height: 20),
            Obx(() => Slider(value: tempVal.value.toDouble().clamp(1, 254), min: 1, max: 254, onChanged: (v) => tempVal.value = v.toInt())),
          ],
        ),
        actions: [
          TextButton(onPressed: () { _sendScenarioValueDelete(sceneId); Get.back(); }, child: Text("sil".tr.toUpperCase(), style: TextStyle(color: theme.colorScheme.primary, fontWeight: FontWeight.bold))),
          TextButton(onPressed: () { _sendScenarioValueUpdate(sceneId, tempVal.value); Get.back(); }, child: Text("kayit".tr.toUpperCase(), style: TextStyle(color: theme.colorScheme.primary, fontWeight: FontWeight.bold))),
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ElevatedButton.icon(onPressed: () => _sendDaliGetConfigCommand("get_detail"), icon: const Icon(Icons.refresh, size: 18), label: Text("full_refresh".tr, style: const TextStyle(fontSize: 12)), style: ElevatedButton.styleFrom(backgroundColor: theme.colorScheme.surface, foregroundColor: theme.colorScheme.primary, elevation: 1, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)))),
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildFadeSectionHeader(title: "extended_fade_settings".tr, onRefresh: () => _sendDaliGetConfigCommand("get_detail"), onSave: () => _sendDaliConfigCommand("set_efade", (extendedFadeTimeMultiplier.value << 4) | (extendedFadeTimeBase.value & 0x0F))),
                const SizedBox(height: 8),
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: DropdownButtonHideUnderline(child: DropdownButton<int>(value: value.clamp(0, items.length - 1), isExpanded: true, items: List.generate(items.length, (index) => DropdownMenuItem(value: index, child: Text(items[index], style: const TextStyle(fontSize: 14)))), onChanged: onChanged)),
        ),
      ],
    );
  }
  ...
  Widget _buildConfigSlider({required String label, required RxInt value, required VoidCallback onApply, required VoidCallback onRefresh, RxInt? minRx}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(padding: const EdgeInsets.only(left: 8.0, top: 8.0), child: Row(children: [Text(label, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)), const SizedBox(width: 8), Obx(() => Text("(${value.value})", style: TextStyle(fontSize: 11, color: Get.theme.colorScheme.primary.withValues(alpha: 0.7))))])),
        Row(
          children: [
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(child: Obx(() => Text(name ?? 'socket'.tr, style: TextStyle(color: theme.colorScheme.onSurface, fontSize: 18, fontWeight: FontWeight.bold)))),
              ],
            ),
            const SizedBox(height: 15),
            Row(
          ...
          children: [
            Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Obx(() { final room = userManager.activeRooms.firstWhereOrNull((r) => r.id == roomId); return _buildInfoChip(Icons.meeting_room, room?.name ?? 'unassigned_room'.tr, theme); }),
                  const SizedBox(width: 10),
                  Obx(() { final sw = userManager.activeSwitches.firstWhereOrNull((s) => s.id == logicButtonId); return _buildInfoChip(Icons.settings_remote, sw?.name ?? 'all_switches'.tr, theme); }),
                ],
              ),
            ),
            Obx(() {
              ...
              return Image.asset(
                'assets/images/priz.png',
                width: 60,
                height: 60,
                color: activeColor.withValues(alpha: isOn ? 1.0 : 0.2),
            ...
            Wrap(
              ...
              children: [
                ...
                _buildActionBtn(Icons.vertical_align_top, 'maximum'.tr, () => _sendMaxCommand()),
                _buildActionBtn(Icons.vertical_align_bottom, 'minimum'.tr, () => _sendMinCommand()),
                _buildActionBtn(Icons.fingerprint, 'identify'.tr, () => _sendIdentifyCommand()),
              ],
            ),
          ],
        ),
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    _buildBottomIconWithLabel(Icons.list_alt, 'details'.tr, () => _showSocketDetailsPopup()),
  ...
  Widget _buildInfoChip(IconData icon, String label, ThemeData theme) {
    return Container(
      ...
      decoration: BoxDecoration(color: theme.colorScheme.primary.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(12), border: Border.all(color: theme.colorScheme.primary.withValues(alpha: 0.1))),
      child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 14, color: theme.colorScheme.primary.withValues(alpha: 0.6)), const SizedBox(width: 4), Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: theme.colorScheme.primary.withValues(alpha: 0.8)))]),
    );
  }
  ...
  void _hideDevice(BuildContext context) {
    Get.defaultDialog(
      title: 'gizle'.tr, middleText: 'device_hide_confirm'.tr, textConfirm: 'yes'.tr, textCancel: 'no'.tr, confirmTextColor: Colors.white,
      onConfirm: () { Get.find<UserManagementService>().updateDeviceIcon(id, channel, 11); Get.back(); Get.back(); },
    );
  }
  ...
  void _showRoomSelectionDialog(BuildContext context) {
    ...
    Get.bottomSheet(
      Container(
        padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: theme.scaffoldBackgroundColor, borderRadius: const BorderRadius.vertical(top: Radius.circular(20))),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
    ...
    Get.bottomSheet(
      Container(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('get_switches'.tr, style: TextStyle(color: theme.colorScheme.onSurface, fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 15),
            if (availableSwitches.isEmpty) Padding(padding: const EdgeInsets.all(20.0), child: Text('no_device_found'.tr))
            else Flexible(child: ListView.builder(shrinkWrap: true, itemCount: availableSwitches.length, itemBuilder: (context, index) { final sw = availableSwitches[index]; final bool isSelected = logicButtonId == sw.id; return ListTile(leading: Icon(Icons.settings_remote, color: isSelected ? theme.colorScheme.primary : Colors.grey), title: Text(sw.name, style: TextStyle(color: theme.colorScheme.onSurface, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)), trailing: isSelected ? Icon(Icons.check_circle, color: theme.colorScheme.primary) : null, onTap: () { _updateDeviceLogicSwitch(sw.id); _sendSaveDeviceToBox(); Get.back(); Get.back(); _showDetailDialog(Get.context!); }); })),
            ListTile(leading: const Icon(Icons.link_off, color: Colors.red), title: Text('delete_switch'.tr, style: const TextStyle(color: Colors.red)), onTap: () { _updateDeviceLogicSwitch(255); _sendSaveDeviceToBox(); Get.back(); Get.back(); _showDetailDialog(Get.context!); }),
          ],
        ),
      ),
    );
  ...
  Widget buildWidget() {
    return Obx(() {
      ...
      return DeviceCardBase(
        ...
        onTap: () {
          if (isOn) _sendOffCommand();
          else { if (!_isStatusFetched) _sendGetLevelCommand(); else _sendActionCommand(); }
        },
