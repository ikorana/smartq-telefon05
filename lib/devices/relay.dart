...
class RelayDevice extends BaseDevice {
  ...
  }) : super(type: DeviceType.relay.typeCode);

  RelayDevice.fromMap(Map<String, dynamic> map) : super.fromMap(map) {
  ...
  void handleIncomingValue(dynamic value) {
    ...
    if (value is Map && value.containsKey('low') && value.containsKey('high')) {
      ...
      debugPrint("--- [RELAY] Grup Verisi İşleniyor: Low: $newLow, High: $newHigh ---");
    ...
    if (newVal != null) {
      debugPrint("--- [RELAY] Durum Güncellendi: Adres $id, Yeni Değer: $newVal ---");
      level.value = newVal;
      _isStatusFetched = true;
    }
  }
  ...
  void _sendSaveDeviceToBox() {
    debugPrint("--- [RELAY] save_dev Komutu Gönderiliyor: Adres $id ---");
    _sendToOutbox({
      "com": "save_dev",
      "id": id,
      "kanal": channel,
  ...
  void _sendOffCommand() {
    debugPrint("--- [RELAY] Off Komutu Gönderiliyor: Adres $id, Kanal $channel ---");
    _sendToOutbox({
      "com": "off",
      "adres": id,
      "gurup": 0,
  ...
  void _sendOpenCommand() {
    debugPrint("--- [RELAY] Open (ArcPower 200) Komutu Gönderiliyor: Adres $id, Kanal $channel ---");
  ...
  void _sendActionCommand() {
    ...
    debugPrint("--- [RELAY] Action (Toggle/Preset) Komutu Gönderiliyor: Adres $id, Kanal $channel ---");
    _sendToOutbox({
      "com": "action",
      "adres": id,
      "gurup": 0,
  ...
  void _showStatusDialog() {
    ...
    Get.dialog(
      AlertDialog(
        title: Text("${name ?? 'relay'.tr} - ${'status'.tr}"),
        content: Obx(() => Column(
    ...
    Get.dialog(
      AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () => Get.back(),
            child: Text("close".tr.toUpperCase(), style: const TextStyle(color: Colors.grey, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(10),
            border: Border.all(color: Get.theme.colorScheme.primary.withValues(alpha: 0.3)),
          ),
          child: DropdownButtonHideUnderline(
            child: DropdownButton<int>(
              value: value.clamp(0, items.length - 1),
              isExpanded: true,
              items: List.generate(items.length, (index) => DropdownMenuItem(value: index, child: Text(items[index], style: const TextStyle(fontSize: 14)))),
              onChanged: onChanged,
            ),
          ),
        ),
          ...
          children: List.generate(lampIcons.length, (index) {
            ...
            return InkWell(
              ...
              child: Container(
                ...
                child: Icon(lampIcons[index], color: isSelected ? theme.colorScheme.primary : theme.colorScheme.onSurface, size: 30),
              ),
            );
          }),
        ),
  ...
  void _updateDeviceLogicSwitch(int switchId) {
    ...
    if (index != -1) {
      ...
      debugPrint("--- [RELAY] Cihaz Anahtar Bağlantısı Güncellendi: Adres $id, Switch: $switchId ---");
    }
  }
