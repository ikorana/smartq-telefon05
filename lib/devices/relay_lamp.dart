...
class RelayLampDevice extends BaseDevice {
  ...
  void _showDetailDialog(BuildContext context) {
    ...
    Get.dialog(AlertDialog(backgroundColor: theme.scaffoldBackgroundColor, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), titlePadding: const EdgeInsets.fromLTRB(20, 20, 20, 0), title: Column(children: [Row(children: [Container(width: 32, height: 32, alignment: Alignment.center, decoration: BoxDecoration(color: theme.colorScheme.primary.withValues(alpha: 0.1), shape: BoxShape.circle, border: Border.all(color: theme.colorScheme.primary.withValues(alpha: 0.3))), child: Text(id.toString(), style: TextStyle(color: theme.colorScheme.primary, fontSize: 13, fontWeight: FontWeight.bold), ), ), const SizedBox(width: 12), Expanded(child: Obx(() => Text(name ?? 'lamp'.tr, style: TextStyle(color: theme.colorScheme.onSurface, fontSize: 18, fontWeight: FontWeight.bold)))), ], ), const SizedBox(height: 15), Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [_buildTopBarIconBtn(Icons.edit_note, 'rename'.tr, () => _showRenameDialog(context)), _buildTopBarIconBtn(Icons.meeting_room_outlined, 'change_room'.tr, () => _showRoomSelectionDialog(context)), _buildTopBarIconBtn(Icons.visibility_off_outlined, 'gizle'.tr, () => _hideDevice(context)), _buildTopBarIconBtn(Icons.settings_remote, 'get_switches'.tr, () => _showSwitchSelectionDialog()), ], ), const Divider(), ], ), content: Column(mainAxisSize: MainAxisSize.min, children: [Padding(padding: const EdgeInsets.only(bottom: 15), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [Obx(() { final room = userManager.activeRooms.firstWhereOrNull((r) => r.id == roomId); return _buildInfoChip(Icons.meeting_room, room?.name ?? 'unassigned_room'.tr, theme); }), const SizedBox(width: 10), Obx(() { final sw = userManager.activeSwitches.firstWhereOrNull((s) => s.id == logicButtonId); return _buildInfoChip(Icons.settings_remote, sw?.name ?? 'all_switches'.tr, theme); })], ), ), Obx(() { final bool isOn = tempLevel.value > 0; return InkWell(onTap: () => _changeIcon(context), borderRadius: BorderRadius.circular(40), child: Icon(_getIconData(ico), size: 60, color: activeColor.withValues(alpha: isOn ? 1.0 : 0.2), ), ); }), const SizedBox(height: 20), Obx(() => Column(children: [Text("%${((tempLevel.value / 254) * 100).toInt()}", style: TextStyle(color: theme.colorScheme.primary, fontSize: 20, fontWeight: FontWeight.bold)), Slider(value: tempLevel.value.toDouble().clamp(0, 254), min: 0, max: 254, onChanged: (v) => tempLevel.value = v.toInt(), onChangeEnd: (v) => _sendCommand(v.toInt())), ], )), const SizedBox(height: 10), Wrap(spacing: 10, runSpacing: 10, alignment: WrapAlignment.center, children: [_buildActionBtn(Icons.power_settings_new, 'open'.tr, () => _sendOpenCommand(), color: Colors.green), _buildActionBtn(Icons.power_settings_new, 'close'.tr, () => _sendOffCommand(), color: Colors.red), _buildActionBtn(Icons.vertical_align_top, 'maximum'.tr, () => _sendMaxCommand()), _buildActionBtn(Icons.vertical_align_bottom, 'minimum'.tr, () => _sendMinCommand()), _buildActionBtn(Icons.fingerprint, 'identify'.tr, () => _sendIdentifyCommand()), ], ), ], ), actionsPadding: EdgeInsets.zero, actions: [Column(mainAxisSize: MainAxisSize.min, children: [const Divider(height: 1), Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [_buildBottomIconWithLabel(Icons.refresh, 'get_level'.tr, () => _sendGetLevelCommand()), _buildBottomIconWithLabel(Icons.info_outline, 'status'.tr, () { _showStatusDialog(); _sendQStatusCommand(); }), _buildBottomIconWithLabel(Icons.list_alt, 'details'.tr, (channel == 10 || channel == 11) ? null : () => _showRelayDetailsPopup()), _buildBottomIconWithLabel(Icons.groups, 'groups'.tr, (channel == 10 || channel == 11) ? null : () { _showGroupsDialog(); _sendGetGroupsCommand(); }), _buildBottomIconWithLabel(Icons.auto_awesome, 'scenarios'.tr, (channel == 10 || channel == 11) ? null : () => _showScenariosDialog()), ], ), ), const Divider(height: 1), TextButton(onPressed: () => Get.back(), child: Text("close".tr.toUpperCase(), style: TextStyle(color: theme.colorScheme.primary)), ), const SizedBox(height: 4), ], ), ], ), ).then((_) => worker.dispose());
  }
  ...
  Widget _buildBottomIconWithLabel(IconData icon, String label, VoidCallback? onTap) {
    ...
    return InkWell(onTap: onTap, child: Opacity(opacity: isEnabled ? 1.0 : 0.3, child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 24, color: Get.theme.colorScheme.onSurface.withValues(alpha: 0.7)), const SizedBox(height: 4), Text(label, style: TextStyle(fontSize: 8, color: Get.theme.colorScheme.onSurface.withValues(alpha: 0.7))), ], ), ), );
  }
